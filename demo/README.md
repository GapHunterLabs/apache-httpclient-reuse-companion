# Demo data — Apache HttpClient Reuse Companion

For capturing the real Marketplace screenshot:

1. `./gradlew runIde`
2. Open `demo/src/main/java/com/acmecorp/payments/PaymentGateway.java`
   as a scratch/standalone file (or drop it into any sandbox project)
   inside the sandbox IDE.
3. The `HttpClients.createDefault()` call inside `charge` shows the
